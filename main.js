var CONTRACT_ADDRESS = "TPiVCZgJePQ5R4WR4wcbnteT611z47tj88";
var SOFT_LAUNCH = 1609462800; 
var FULL_LAUNCH = 1609491600; 
...
function load_contract_details(){
  window.sc.contract.contractInfo().call()
...
function load_account_details(){
  if(window.sc.address != undefined && window.sc.address != null){
    window.sc.contract.userInfo(window.sc.address).call()
      .then(function(response){
...
function load_account_balance(){
  if(window.sc.address != undefined && window.sc.address != null){
    window.tronWeb.trx.getAccount(window.sc.address)
    .then(function(response){
...
function load_account_payments(){
  if(window.sc.address != undefined && window.sc.address != null){
    window.sc.contract.investmentsInfo(window.sc.address).call()
    .then(function(response){
...
function deposit(value){
  window.sc.contract.deposit(window.referral).send({ callValue: value * 1000000 })
  .then(function(response){
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  var load_obj = setInterval(async ()=>{
      if (window.tronWeb && window.tronWeb.defaultAddress.base58) {
        ...
        window.sc.address = window.tronWeb.defaultAddress.base58;
        window.sc.contract = await window.tronWeb.contract().at(CONTRACT_ADDRESS)
          .catch(function(err){ swal("Oh no :/", err, "error") })
        ...
        document.getElementById("your_address").innerHTML = window.sc.address
        document.getElementById("referral_link").value = "https://tron7.me?r="+window.sc.address;
        document.getElementById("your_adr").innerHTML = "<i class='fa fa-lock mr-2 text-success'></i> "+window.sc.address.substring(0,12)+"...";
  ...
  window.setInterval(function(){
    if(window.sc.address != window.tronWeb.defaultAddress.base58){
      window.sc.address = window.tronWeb.defaultAddress.base58;
      document.getElementById("your_address").innerHTML = window.sc.address;
      document.getElementById("referral_link").value = "https://tron7.me?r="+window.sc.address;
      document.getElementById("your_adr").innerHTML = "<i class='fa fa-lock mr-2 text-success'></i> "+window.sc.address.substring(0,12)+"...";
  ...
  document.getElementById("withdraw").addEventListener('click', async (e)=>{
	...
	if(new Date().getTime() < FULL_LAUNCH * 1e3) {
		...
		window.sc.contract.withdraw().send()
		.then(response => swal("Withdrawn!", "Successfully withdrawn balance.", "success"))
		.catch(function(err){ swal("Oh no :/", err, "ERROR") })
	}
  });
  // document.getElementById("reinvest").addEventListener('click', async (e)=>{
  //   e.preventDefault();
  //   window.sc.contract.reinvest().send()
